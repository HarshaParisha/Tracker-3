      ...
      <Card title="Add Opportunity" subtitle="Log application details and target company" color="cream">
        <form onSubmit={handleAddApplication} className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="submit"
              className="flex items-center gap-1.5 rounded-full bg-[#ff4d8b] px-5 py-2.5 text-xs font-bold text-white hover:opacity-90 transition shrink-0 shadow-sm"
            >
            ...
            <tbody className="divide-y divide-[var(--hairline)]">
              {filteredApps?.map((app) => (
                <tr key={app.id} className="hover:bg-[var(--surface-soft)] transition">
                  <td className="py-3 font-mono text-[11px] font-bold text-[var(--text-muted)]">{app.date}</td>
                  <td className="py-3 font-extrabold text-[var(--ink)]">{app.company}</td>
                  <td className="py-3 text-[var(--text-body)] font-medium">{app.role}</td>
                  <td className="py-3">
                    <select
                      value={app.status}
